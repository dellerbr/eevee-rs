...
| 6, integral data | Four-state packed scalar/vector storage; represented signedness, sizing, and assignment coercion | `behaviorally validated` | `LogicVec` and strict source tests cover narrow/wide X/Z values, signed/unsigned extension and truncation, mixed/all-signed arithmetic, signed/mixed relational operators, unsized signed decimals, arithmetic `>>>`, constant-fold parity, procedural/class/static/package/callable-return destinations, continuous assignments, and directional input/output port conversion. Integral casts, callable argument copy-in/out coercion, streaming, multiple packed dimensions, and complete IEEE type propagation remain unsupported. |
| 7, aggregate data | Queues/associative arrays used by the class runtime; one fixed unpacked unsigned integral module-memory dimension | `behaviorally validated` | Collection tests cover indexing, copy, methods, object keys, and `foreach`. Strict RTL tests cover parameterized ascending/descending nonnegative memory bounds, per-element scheduler storage, dynamic reads, blocking/NBA writes, continuous sensitivity, and X/ignored-write behavior for invalid indices. Multiple dimensions, signed/class/string/event memory elements, declaration initialization, memory ports, whole-memory assignment, signed/negative indices, and procedural/class/package fixed arrays remain unsupported. |
...
| 10, assignments | Procedural blocking/nonblocking integral assignments with destination coercion; static continuous-driver Z withdrawal | `behaviorally validated` | Procedural and scheduler tests include truncation, source-signed extension, locals/fields/callable returns, clocked NBA updates, and simultaneous swaps that read old values before NBA settling. Static continuous assignments coerce width/sign and can withdraw their driver contribution by producing Z. Callable argument copy-in/out coercion, procedural `assign/deassign`, and `force/release` remain unsupported. |
| 11, expressions | Represented signed/unsigned arithmetic, bitwise, logical, relational, logical/arithmetic shift, reduction, dynamic bit select, constant part-select, concatenation, and integral conditional operators; signed decimals and unbased unsized fill literals | `behaviorally validated` | Core and strict runtime tests cover signed/mixed operand context, signed relational X propagation, `>>>`, four-state conditional truth/merging, destination-context fills, dynamic packed reads/writes, and invalid-index X/ignored writes. Integral casts, streaming, dynamic/indexed part-selects, non-integral conditionals, and the remaining operator/type rules are unsupported. |
...
| 23, connectivity | Named and positional whole-signal connections; matching-resolution collapse; directional resolution and width-converting input/output bridges | `behaviorally validated` | Strict end-to-end tests check resolved ports across repeated/nested instances plus signed/unsigned narrowing and extension in both input and output directions. Exact compatible ports collapse; bridge storage preserves independent resolution. Mismatched inout/implicit pull-supply bridges, inout/ref width conversion, non-net output/inout actuals, and wired-port strengths fail preflight. Empty, implicit, wildcard, expression, select, interface, and hierarchical actuals remain rejected. Full directionality and complete variable/net port semantics are not claimed. |
| 23, module parameters | Explicit/inherited bare 32-bit `int` value defaults; named/positional overrides; one parameter-dependent packed dimension on module ports, variables, and nets | `behaviorally validated` | Strict tests cover declaration-order dependent defaults, parent-parameter override expressions, independent child initializer and `initial` values, parameterized delays, and sibling instances specializing one child declaration to independent 4-bit and 8-bit ports/storage/nets. Undeclared bound constants fail closed; input/output width differences coerce directionally. Type parameters/actuals, non-`int` parameter types, missing defaults, header/body localparams, module-body parameters, omitted actuals, `defparam`, multiple packed dimensions, and parameter-dependent callable signatures/procedural locals remain unsupported. |
...
| 23/28, continuous connectivity | Internal, collapsed-port, and directionally bridged `wire`/`tri`, `wand`/`triand`, and `wor`/`trior`; implicit pull/supply nets; width/sign-coerced integral whole-net assignments and internal net declarations with optional one-, two-, or three-value timescale-relative inertial delays | `behaviorally validated` | Strict tests cover four-state resolution, symmetric/asymmetric strengths, implicit pull/supply drivers, signed/unsigned width coercion, time-zero evaluation, parameter/transition/declaration delays, inertial cancellation, conditional Z withdrawal, source-specific sensitivity including dynamic reads, resolved-port collapse/bridges, and invalid diagnostics. Net declaration assignments, procedural net writes, charge strengths, nondefault wired-net strengths, explicit-unit delay literals, dynamic part-selects, partial/hierarchical targets, cross-resolution inout or implicit-strength bridges, procedural `assign/deassign`, and `force/release` remain unsupported. |
...
| 27, generate | Integral-constant generate `if`, `case`, and `for`; declared/inline genvars; named indexed scopes | `behaviorally validated` | Strict frontend and end-to-end tests cover branch pruning, multi-label/default case selection, ascending loops, nested conditionals, per-iteration genvar constants, generated variables/nets/continuous assignments, generated parameterized child instances, and hierarchy names such as `lane[1].special.marker`. Unknown modules, hierarchy cycles, nonconstant expressions, undeclared/wrong step genvars, and unnamed blocks fail closed. Implicit `genblk` naming, generate-local parameters, arrays of instances, interfaces, and hierarchical references into generated scopes remain unsupported. |
