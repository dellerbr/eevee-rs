...
| 10, assignments | Procedural blocking/nonblocking subset; static continuous-driver Z withdrawal | `behaviorally validated` | Procedural and scheduler tests include clocked NBA updates and simultaneous swaps that read old values before NBA settling. Static continuous assignments can withdraw their driver contribution by producing Z. Procedural `assign/deassign` and `force/release` remain unsupported. |
| 11, expressions | Represented arithmetic, bitwise, logical, comparison, shift, reduction, dynamic bit select, constant part-select, concatenation, and integral conditional operators; unbased unsized fill literals | `behaviorally validated` | Core tests cover four-state conditional truth, equal-X/Z branch merging, unequal branch widths, and destination-context `'0`/`'1`/`'x`/`'z`. Strict runtime tests cover lazy known branches, both-branch indeterminate evaluation, dynamic packed reads/writes, and X/ignored-write behavior for unknown or out-of-range indices. Non-integral conditionals, dynamic/indexed part-selects, complete IEEE sizing/sign rules, and the remaining operator set are unsupported. |
...
| 23, connectivity | Named and positional whole-signal connections; matching-resolution collapse; one-way ordinary cross-resolution input/output bridges | `behaviorally validated` | Strict end-to-end tests check procedural and continuous propagation plus resolved ports across repeated and nested instances. Canonical synonyms collapse onto one scheduler net. Mismatched `wire`/`wand`/`wor` input and output ports allocate child-local storage and a directional reactive bridge; tests distinguish those nets from aliases, cover variable actual isolation, and exercise timed propagation. Unbound root pull ports install one implicit driver. Mismatched inout and implicit pull/supply bridges, non-net output/inout actuals, and explicit strengths on wired ports fail preflight. Empty, implicit, wildcard, expression, select, interface, hierarchical, and width-converting actuals remain rejected. Full directionality and complete variable/net port semantics are not claimed. |
| 23, module parameters | Explicit/inherited bare 32-bit `int` value defaults; named/positional overrides; one parameter-dependent packed dimension on module ports, variables, and nets | `behaviorally validated` | Strict tests cover declaration-order dependent defaults, parent-parameter override expressions, independent child initializer and `initial` values, parameterized delays, and sibling instances specializing one child declaration to independent 4-bit and 8-bit ports/storage/nets. Resolved-width conversion and undeclared bound constants fail closed. Type parameters/actuals, non-`int` parameter types, missing defaults, header or body localparams, module-body parameters, omitted actuals, `defparam`, multiple packed dimensions, parameter-dependent callable signatures/procedural locals, and complete sizing/signing/coercion remain unsupported. |
