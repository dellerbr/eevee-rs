...
| 11, expressions | Represented arithmetic, bitwise, logical, comparison, shift, reduction, select, concatenation, and integral conditional operators; unbased unsized fill literals | `behaviorally validated` | Core tests cover four-state conditional truth, equal-X/Z branch merging, unequal branch widths, and destination-context `'0`/`'1`/`'x`/`'z`. Strict runtime tests cover lazy known branches and both-branch indeterminate evaluation. Non-integral conditionals, complete IEEE sizing/sign rules, and the remaining operator set are unsupported. |
