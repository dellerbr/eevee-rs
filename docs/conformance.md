...
| 23/28, continuous connectivity | Plain `wire`; undelayed strengthless whole-net assignments; exact-width unsigned represented RHS expressions; multiple-driver resolution | `behaviorally validated` | Strict tests cover time-zero evaluation, deduplicated RHS-source sensitivity, unrelated-write isolation, reactive updates, child-output propagation, in-range literal selects/concatenation, Z identity, 0/1 conflict to X, and invalid driver diagnostics. Constant RHS drivers evaluate once. Net declaration assignments, procedural net writes, implicit width conversion, signed operands/targets, other net types, strengths, assignment delays, dynamic/out-of-range RHS selects, partial/hierarchical targets, and driver release remain unsupported. |
