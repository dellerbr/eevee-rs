...
| 9, procedural processes | `initial`, `always`, timescale-relative constant delays, event waits | `behaviorally validated` | End-to-end/procedural tests include per-instance parameter delay expressions. Explicit-unit delay literals, full process handles, `disable fork`, `wait fork`, and all process control remain unsupported. |
...
| 23/28, continuous connectivity | Plain `wire`; undelayed strengthless whole-net assignments; exact-width unsigned represented RHS expressions; multiple-driver resolution | `behaviorally validated` | Strict tests cover time-zero evaluation, reactive updates, child-output propagation, in-range literal selects/concatenation, Z identity, 0/1 conflict to X, and invalid driver diagnostics. Net declaration assignments, procedural net writes, implicit width conversion, signed operands/targets, other net types, strengths, assignment delays, dynamic/out-of-range RHS selects, partial/hierarchical targets, and driver release remain unsupported. Read-set-specific wakeups are not yet optimized. |
