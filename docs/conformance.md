...
| 9, procedural processes | `initial`, `always`, timescale-relative constant delays, event waits | `behaviorally validated` | End-to-end/procedural tests include per-instance parameter delay expressions. Multi-source waits use generation-safe registration and remove quiet sibling waiters after the first source fires. Explicit-unit delay literals, full process handles, `disable fork`, `wait fork`, and all process control remain unsupported. |
...
| 10, assignments | Procedural blocking and nonblocking assignment subset | `behaviorally validated` | Procedural and scheduler tests. Force/release and procedural continuous assignment are unsupported. |
...
| 23, module declarations | ANSI scalar/packed logic input/output/inout/ref port metadata | `behaviorally validated` | Frontend parser regression checks direction and width. Non-ANSI ports and full port-kind/type rules remain unsupported. |
...
| 23, connectivity | Named and positional whole-signal connections with parent/child propagation | `behaviorally validated` | Strict end-to-end tests check procedural and continuous child-output propagation. Empty, implicit, wildcard, expression, select, interface, hierarchical, and width-converting actuals are rejected. Ports currently alias one scheduler net; full directionality and complete variable/net port semantics are not claimed. |
...
| 23/28, continuous connectivity | Internal `wire`/`tri`, `tri0`/`tri1`, `supply0`/`supply1`, `wand`/`triand`, and `wor`/`trior`; default-strong whole-net assignments with zero or one timescale-relative inertial delay; exact-width unsigned represented RHS expressions | `behaviorally validated` | Strict tests cover ordinary, wired-AND, and wired-OR four-state resolution; symmetric `Weak < Pull < Strong < Supply` ordinary-net resolution; implicit full-width pull/supply drivers; time-zero/zero-delay evaluation; per-instance parameter delay expressions; inertial pulse rejection; identical-request preservation; canceled-event pruning; deduplicated RHS-source sensitivity; unrelated-write isolation; child-output propagation; in-range literal selects/concatenation; and invalid driver diagnostics. Constant RHS drivers evaluate once. Resolved net ports, net declaration assignments/delays, procedural net writes, implicit width conversion, signed operands/targets, explicit/asymmetric drive strengths, charge strengths, nondefault wired-net strengths, rise/fall/turn-off delay tuples, explicit-unit delay literals, dynamic/out-of-range RHS selects, partial/hierarchical targets, and driver release remain unsupported. |
...
| 28, strength modeling | Symmetric weak/pull/strong/supply resolution on ordinary nets and implicit `tri0`/`tri1`/`supply0`/`supply1` drivers | `behaviorally validated` | Scheduler tests cover all-Z, weak-versus-pull, strong, supply, equal-strength conflict, and strength-qualified X behavior. Strict source-to-runtime tests cover scalar and vector implicit drivers plus strong-versus-pull/supply behavior. Explicit/asymmetric strength syntax, charge strengths, and nondefault strengths on wired nets remain unsupported. |
