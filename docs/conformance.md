...
| 9, procedural processes | `initial`, `always`, delays, event waits | `behaviorally validated` | End-to-end/procedural tests. Full process handles, `disable fork`, `wait fork`, and all process control remain unsupported. |
...
| 23, connectivity | Named and positional whole-signal connections with parent/child propagation | `behaviorally validated` | Strict end-to-end test checks delayed input-to-output behavior. Empty, implicit, wildcard, expression, select, interface, hierarchical, and width-converting actuals are rejected. Ports currently alias one scheduler net; full directionality, variable/net distinctions, drivers, and resolution are not claimed. |
| 23, module parameters | Parameter declarations, defaults, and instance overrides | `unsupported` | Strict frontend rejects module parameterization. |
...
| 23/28, continuous connectivity | Continuous assignments, net drivers, strengths, delays, resolution | `unsupported` | Strict frontend regression rejects `assign` with a source coordinate. |
...
| 28, gate/switch modeling | Primitives, strengths, delays, resolution | `unsupported` | No conformance claim. |
